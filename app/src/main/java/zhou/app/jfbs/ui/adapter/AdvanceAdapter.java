...
public class AdvanceAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AdvanceAdapter(@Nullable RecyclerView.Adapter innerAdapter) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType < HEAD_START + headCount) {
            ...
            return innerAdapter == null ? null : innerAdapter.onCreateViewHolder(parent, viewType - Integer.MAX_VALUE / 2);
        }
    }
    ...
    public int getItemCount() {
        return headers.size() + footers.size() + (innerAdapter == null ? 0 : innerAdapter.getItemCount());
    }
    ...
    public int getItemViewType(int position) {
        int innerCount = innerAdapter == null ? 0 : innerAdapter.getItemCount();
        int headCount = headers.size();
        if (headCount > position) {
            return HEAD_START + position;
        } else if (headCount <= position && headCount + innerCount > position) {
            return innerAdapter == null ? 0 : innerAdapter.getItemViewType(position - headCount) + Integer.MAX_VALUE / 2;//此处可能发生溢出
        } else {
